...
SQL <- 'select *, year(Stichtag) + 1 as Jahr from DT124110006 order by `Stichtag`, `Geschlecht`, `Alter`;'

DT124110006 <- RunSQL( SQL )

DT124110006$Geschlecht <- factor(DT124110006$Geschlecht,levels = c( 'F','M'), labels = c('Frauen','Männer'))

  DT124110006 %>% filter( year(Stichtag) >= 2016 & Alter >= 65 & Alter < 85 ) %>% ggplot(
      aes( x = Alter, y = Einwohner, group = Geschlecht, colour = Geschlecht)) +
      geom_line( alpha = 0.7 ) +
      geom_smooth() + 
    ...
    ggsave(   filename = paste(outdir, 'DT124110006_AG70-84.png', sep='')
            , plot = P
            , device = "png"
            , bg = "white"
