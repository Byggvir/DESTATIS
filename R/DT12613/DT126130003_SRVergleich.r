  ...
  for ( GE in c('Männer', 'Frauen') ){
    ...
    xy %>% ggplot(
      ...
      geom_smooth( method = glm
                   , formula = 'y ~ x'
                   , aes( colour = 'smoothed' ), alpha = 0.2 ) +
      geom_point( aes( colour = 'Sterberate' ) ) +
      # geom_text( aes( label = Alter, colour = 'Alter' )
      #            , size = 3 
      #            # , max.overlaps = 100
      #            ) +
      ...
      labs(  title = paste( 'Sterberate: Alter von', minA, 'bis', maxA, 'Jahre')
    ...
    data %>% filter( (Geburtsjahr == G1 | Geburtsjahr == G2 ) & Alter > 0 & Geschlecht == GE ) %>% ggplot(
      aes( x = Alter, y = Gestorbene / Einwohner , colour = Jahrgang )
    ) +
      geom_line( ) +
      #      scale_x_continuous( labels = function(x) format(x, big.mark = ".", decimal.mark= ',', scientific = FALSE ) ) +
      scale_y_continuous( labels = function(x) format(x, big.mark = ".", decimal.mark= ',', scientific = FALSE ) ) +
      ...
      labs(  title = paste( "Sterberate nach Alter" )
             , subtitle = paste( "Vergleich der Geburtsjahrgänge", G1 ,"und", G2 )
             , x = paste( "Alter" )
             , y = paste( "Sterberate" )
             , colour = 'Geburtsjahrgang'
             , caption = '' )  -> P2

    PX <- grid.arrange( P1
                        , P2
                        , nrow = 1
                        , top = textGrob(  paste("Jährliche Sterberate", GE)
                                           , gp = gpar( fontsize = 12, font = 3 ))
                        , bottom = textGrob( citation
                                             , gp = gpar( fontsize = 6, font = 3 ))
    )

    ggsave(  paste(outdir, GE, '_', G1, '_', str_pad(delta, 2, pad = "0"),'.png', sep = '' )
             , plot = PX
             , device = "png"
             , bg = "white"
             , width = 1920
